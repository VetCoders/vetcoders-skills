...
This plugin implements Marbles using a **Stop hook** that intercepts Claude's exit attempts:

```bash
# You run ONCE:
/marbles "Your task description" --completion-promise "DONE"
...
```bash
/marbles "Build a REST API for todos. Requirements: CRUD operations, input validation, tests. Output <promise>COMPLETE</promise> when done." --completion-promise "COMPLETE" --max-iterations 50
```
...
**Usage:**
```bash
/marbles "<prompt>" --max-iterations <n> --completion-promise "<text>"
```
...
/marbles "Try to implement feature X" --max-iterations 20
...
Marbles embodies several key principles:
